...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="每行格子数 (default: 5)",
    )

    parser.add_argument(
        "-p", "--paper", default="us_letter", choices=PAPER_SIZE_OPTIONS, metavar="SIZE"
    )
    ...
    output_group.add_argument(
        "-v", "--version", action="version", version=f"chinese-chars 0.1.0"
    )
    return parser
...
def _validate_chars(chars: str) -> str:
    ...
    if not chars or chars.strip() == "":
        raise argparse.ArgumentTypeError(
            "需要至少一个汉字 Please provide at least one character."
        )
...
def _validate_paper(paper: str) -> str:
    ...
    if paper not in PAPER_SIZE_OPTIONS:
        raise argparse.ArgumentTypeError(
            f"Invalid paper size '{paper}'. Use us_letter or a4."
        )
    return paper
